...
class InteractiveRobotPositioner(Node):
    def __init__(self):
        ...
        map_folder = self.get_parameter('map_folder').get_parameter_value().string_value
        
        # Determinar pasta para salvar waypoints
        if map_folder and map_folder != '':
            # Salvar na pasta da arena específica
            self.waypoints_file = os.path.join(map_folder, 'waypoints.json')
        else:
            # Fallback: pasta padrão do config
    ...
    def save_waypoint(self, waypoint):
        ...
        self.get_logger().info(f"💾 Waypoint salvo em: {self.waypoints_file}")
